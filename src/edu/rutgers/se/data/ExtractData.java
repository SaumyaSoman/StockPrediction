...
public class ExtractData {
	...
	private void saveData(JSONObject jsonObject) {
		...
		try {
			...
			for (int i=0;i<quotes.length();i++) {
				...
				String sql = "INSERT INTO realtimedata VALUES (\'"+quote.getString("Symbol")+"\', "+quote.getDouble("LastTradePriceOnly")+", DATE_FORMAT( NOW(),'%Y-%m-%d') ,  "
						+ "TIME_FORMAT(NOW(),'%H:%i:%s') , "+quote.getLong("Volume")+", "+quote.getDouble("Change")+", \'"+quote.getString("PercentChange")+"\')";
	...
	private JSONObject collectData() {
		...
		JSONObject jsonObject=null;
		try {
			url = new URL("https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%20in%20(%22YHOO%22%2C%22AMZN%22%2C%22GOOG%22%2C%22EBAY%22%2C%22BBY%22%2C%22MSFT%22%2C%22FB%22%2C%22TWTR%22%2C%22AAPL%22%2C%22LNKD%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=");
